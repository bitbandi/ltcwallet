...
func TstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   luxutil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount luxutil.Amount,
	net *chaincfg.Params) OutputRequest {
	addr, err := luxutil.DecodeAddress(address, net)
	if err != nil {
...
func TstConstantFee(fee luxutil.Amount) func() luxutil.Amount {
	return func() luxutil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	dustThreshold := luxutil.Amount(1e4)
