...
import (
	...
	"github.com/bitbandi/luxwallet/internal/zero"
...
// Address returns the luxutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() luxutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*luxutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return luxutil.NewWIF(pk, a.manager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey *btcec.PublicKey,
	...
	if compressed {
		...
		pubKeyHash = luxutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address luxutil.Address
	var err error
	...
	case adtChainNestedWitness:
		...
		witAddr, err := luxutil.NewAddressWitnessPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
		...
		address, err = luxutil.NewAddressScriptHash(witnessProgram, m.chainParams)
		if err != nil {
			return nil, err
		}
	case adtImport:
		...
		fallthrough
	case adtChain:
		address, err = luxutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
	case adtChainWitness:
		address, err = luxutil.NewAddressWitnessPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
	}
...
type scriptAddress struct {
	manager         *Manager
	account         uint32
	address         *luxutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() luxutil.Address {
	return a.address
}
