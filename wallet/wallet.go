...
import (
	...
	"github.com/bitbandi/luxwallet/wallet/txrules"
...
func (w *Wallet) RelayFee() luxutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee luxutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]luxutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []luxutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr luxutil.Address) error {
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []luxutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (luxutil.Amount, error) {
...
type Balances struct {
	Total          luxutil.Amount
	Spendable      luxutil.Amount
	ImmatureReward luxutil.Amount
}
...
func (w *Wallet) CurrentAddress(account uint32) (luxutil.Address, error) {
	var addr luxutil.Address
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) PubKeyForAddress(a luxutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a luxutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) HaveAddress(a luxutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a luxutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a luxutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal luxutil.Amount
		for _, deb := range details.Debits {
			debitTotal += deb.Amount
		}
		var outputTotal luxutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += luxutil.Amount(output.Value)
		}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := luxutil.Amount(output.Value).ToBTC()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*luxutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*luxutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr luxutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr luxutil.Address) (string, error) {
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *luxutil.WIF, bs *waddrmgr.BlockStamp,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]luxutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32, addrType waddrmgr.AddressType) (addr luxutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	addrType waddrmgr.AddressType) (luxutil.Address, error) {
	...
	utilAddrs := make([]luxutil.Address, len(addrs))
...
func (w *Wallet) NewChangeAddress(account uint32, addrType waddrmgr.AddressType) (addr luxutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, addrType waddrmgr.AddressType) (luxutil.Address, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr luxutil.Address, minConf int32) (luxutil.Amount, error) {
	var amount luxutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*luxutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr luxutil.Address) (*btcec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr luxutil.Address) ([]byte, error) {
