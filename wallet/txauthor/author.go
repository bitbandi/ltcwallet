...
import (
	...
	h "github.com/bitbandi/luxwallet/internal/helpers"
	"github.com/bitbandi/luxwallet/wallet/internal/txsizes"
)
...
// InputSourceError.
type InputSource func(target luxutil.Amount) (total luxutil.Amount, inputs []*wire.TxIn,
	inputValues []luxutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []luxutil.Amount
	TotalInput      luxutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb luxutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []luxutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := luxutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
