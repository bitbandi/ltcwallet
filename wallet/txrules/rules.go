...
const DefaultRelayFeePerKb luxutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb luxutil.Amount) luxutil.Amount {
	...
	relayFee := luxutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount luxutil.Amount, scriptSize int, relayFeePerKb luxutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb luxutil.Amount) bool {
	...
	return IsDustAmount(luxutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb luxutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > luxutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb luxutil.Amount, txSerializeSize int) luxutil.Amount {
	fee := relayFeePerKb * luxutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > luxutil.MaxSatoshi {
		fee = luxutil.MaxSatoshi
	}
